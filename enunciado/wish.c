...
void check_and_execute_sh(char *expresion, char **path) {
    ...
    if (strstr(expresion, ".sh") != NULL) {
        ...
        // Crear un array para los argumentos
        ...
        if (access(filepath, X_OK) == 0) {
            ...
            execvp(filepath, args);  // Pasamos 'args' que es un array terminado en NULL
        } else {
            printf("No se pudo ejecutar el script: permiso denegado o no existe\n");
        }
